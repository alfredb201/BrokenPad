...
class BackgroundElements {
    ...
    func createMovingGround(gameScene: GameScene) -> SKSpriteNode{
        let groundTexture = SKTexture(imageNamed: "GameGround")
        let ground = SKSpriteNode(texture: groundTexture)
        ground.position = CGPoint(x: gameScene.frame.width + ground.size.width, y: groundTexture.size().height / 2)
        ground.zPosition = 0
        ground.physicsBody = SKPhysicsBody(texture: ground.texture!, size: ground.texture!.size())
        ground.physicsBody?.categoryBitMask = PhysicsCategory.ground.rawValue
        ground.physicsBody?.isDynamic = false
    ...
    func startGround(){
        ...
        let moveLeft = SKAction.move(to: CGPoint(x: -groundElement.size.width, y: groundElement.position.y), duration: 10)
        let moveReset = SKAction.removeFromParent()
        let moveLoop = SKAction.sequence([moveLeft, moveReset])
    ...
    func addHole(gameScene: GameScene) -> SKSpriteNode{
        ...
        let moveAction = SKAction.move(to: CGPoint(x: -hole.size.width/2, y: groundHeight / 2), duration: TimeInterval(10))
        let moveActionDone = SKAction.removeFromParent()
